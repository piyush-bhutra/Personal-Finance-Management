...
export default function ExpensesPage() {
  ...
  const validateCreate = () => {
    ...
    if (mode === "recurring") {
      ...
      if (!Number.isFinite(m) || m <= 0 || m > MAX_EXPENSE_AMOUNT)
        return "Monthly amount must be between 1 and 100000000.";
      if (!formData.startDate || isFutureDate(formData.startDate))
        return "Start date is required and cannot be in the future.";
    } else {
      const a = Number(formData.amount);
      if (!Number.isFinite(a) || a <= 0 || a > MAX_EXPENSE_AMOUNT)
        return "Amount must be between 1 and 100000000.";
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not save expense. Please check values and try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (
      ...
      (Number(editData.monthlyAmount) <= 0 ||
        Number(editData.monthlyAmount) > MAX_EXPENSE_AMOUNT)
    )
      return setFormError("Monthly amount must be between 1 and 100000000.");
    if (editTarget.expenseMode === "one-time") {
      if (
        Number(editData.amount) <= 0 ||
        Number(editData.amount) > MAX_EXPENSE_AMOUNT
      )
        return setFormError("Amount must be between 1 and 100000000.");
      if (!editData.date || isFutureDate(editData.date))
        return setFormError("Valid non-future date is required.");
    }
    ...
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not update expense right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const doDelete = async (id, fd) => {
    ...
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not delete expense right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const confirmStop = async () => {
    if (!stopDate || isFutureDate(stopDate))
      return setFormError("Stop date cannot be in the future.");
    setSubmitting(true);
    setFormError("");
    setApiError("");
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not stop expense right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const expenseSummary = useMemo(() => {
    return expenses.reduce(
      (acc, exp) => {
        const isRecurring = exp.expenseMode === "recurring";
        const amount = Number(
          isRecurring
            ? exp.totalAmount ?? exp.monthlyAmount ?? 0
            : exp.amount ?? 0,
        );
        acc.totalSpent += amount;
        if (isRecurring && exp.status !== "closed") {
          acc.activeRecurring += 1;
          acc.monthlyCommitment += Number(exp.monthlyAmount || 0);
        }
        if (exp.status === "closed") acc.closedCount += 1;
        return acc;
      },
      {
        totalSpent: 0,
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto py-10 px-4 max-w-6xl space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            <CardContent className="p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Active Recurring</p>
              <p className="mt-1 text-xl font-semibold tabular-nums">{expenseSummary.activeRecurring}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Monthly Commitment</p>
              <p className="mt-1 text-xl font-semibold text-primary tabular-nums">
                {formatCurrency(expenseSummary.monthlyCommitment)}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Closed Plans</p>
              <p className="mt-1 text-xl font-semibold tabular-nums">{expenseSummary.closedCount}</p>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div className="space-y-4">
                    {expenses.map((exp) => (
                      <Motion.div
                        layout
                        key={exp._id}
                        className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border rounded-lg bg-card/50 hover:bg-muted/50 transition-colors gap-4"
                      >
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-muted-foreground mt-1">
                            {exp.expenseMode === "recurring" ? (
                              <>
                                <span className="inline-block bg-primary/10 text-primary px-2 py-0.5 rounded text-xs mr-2 border border-primary/20">
                                  Recurring
                                </span>
                                <span>
                                  Started{" "}
                            ...
                            {exp.status === "closed" && (
                              <span className="ml-2 text-primary/80">
                                {" "}
                                • Stopped on {formatDate(exp.stopDate)}
                              </span>
                            )}
                            {exp.description && (
                        ...
                        <div className="flex w-full flex-col items-start gap-2 sm:w-auto sm:items-end">
                          ...
                          <div className="mt-2 flex w-full flex-wrap gap-2 sm:justify-end">
                            {exp.status !== "closed" && (
                              <Button
                                variant="outline"
                                size="sm"
                            ...
                            <Button
                              ...
                              onClick={() =>
                                exp.expenseMode === "recurring"
                                  ? (setPendingDeleteId(exp._id),
                                    setFromDate(nowYM()),
                                    setFromDateOpen(true))
                                  : doDelete(exp._id, null)
                              }
                              className="h-8 px-2"
                            >
        ...
        <Dialog open={fromDateOpen} onOpenChange={setFromDateOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Select Effective Month</DialogTitle>
            </DialogHeader>
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                This ensures historical expense entries remain untouched,
                maintaining accurate past reporting.
              </p>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setFromDateOpen(false)}>
                Cancel
              </Button>
              <Button
                onClick={() => {
                  ...
                  pendingDeleteId
                    ? doDelete(pendingDeleteId, fromDate)
                    : setEditOpen(true);
                  setPendingDeleteId("");
                }}
              >
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                This will stop generating new expense entries from this date
                forward. Past entries will be preserved for accurate historical
                tracking.
              </p>
              <div>
              ...
              <Button
                ...
                disabled={submitting}
              >
                {submitting ? "Processing..." : "Confirm Stop"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
