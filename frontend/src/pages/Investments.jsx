...
export default function InvestmentsPage() {
  ...
  const validateCommon = (assetName, returnRate) => {
    ...
    if (
      returnRate !== "" &&
      (Number(returnRate) < 0 || Number(returnRate) > MAX_RETURN)
    )
      return `Expected return must be between 0 and ${MAX_RETURN}.`;
    return "";
  };
  ...
  const validateCreate = () => {
    ...
    if (mode === "recurring") {
      ...
      if (!Number.isFinite(m) || m <= 0 || m > MAX_AMOUNT)
      ...
      if (!Number.isFinite(a) || a <= 0 || a > MAX_AMOUNT)
        return "Amount must be between 1 and 100000000.";
      if (!formData.date || isFutureDate(formData.date))
        return "Investment date is required and cannot be in the future.";
    }
    return "";
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not save investment. Please check values and try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (editTarget.investmentMode === "recurring")
      ...
      const msg =
        error.response?.data?.message ||
        "Could not update investment right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const doDelete = async (id, fd) => {
    ...
    try {
      ...
      const msg =
        error.response?.data?.message ||
        "Could not delete investment right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const confirmStop = async () => {
    ...
    if (!Number.isFinite(Number(realizedValue)) || Number(realizedValue) < 0)
      return setFormError("Realized value must be 0 or greater.");
    setSubmitting(true);
    setFormError("");
    setApiError("");
    try {
      await investmentService.stopInvestment(stopTarget._id, {
        stopDate,
      ...
      const msg =
        error.response?.data?.message ||
        "Could not stop/sell investment right now. Please try again.";
      setApiError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleExport = () => {
    const exportData = investments.map((inv) => ({
      ...
      Monthly_Amount:
        inv.investmentMode === "recurring" ? inv.monthlyAmount : "N/A",
  ...
  const investmentSummary = useMemo(() => {
    return investments.reduce(
      (acc, inv) => {
        ...
        if (inv.status !== "closed") acc.activeCount += 1;
        if (inv.investmentMode === "recurring" && inv.status !== "closed") {
          acc.monthlySip += Number(inv.monthlyAmount || 0);
        }
        return acc;
      },
      { totalInvested: 0, currentValue: 0, activeCount: 0, monthlySip: 0 },
    );
  }, [investments]);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto py-10 px-4 max-w-6xl space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <p className="mt-1 text-xl font-semibold tabular-nums">{investmentSummary.activeCount}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <p className="text-xs uppercase tracking-wide text-muted-foreground">Monthly SIP</p>
              <p className="mt-1 text-xl font-semibold text-primary tabular-nums">
                {formatCurrency(investmentSummary.monthlySip)}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <Card className="h-fit md:sticky md:top-24 md:col-span-1">
            <CardHeader>
                  ...
                  <CardContent className="py-4 space-y-3">
                    <div className="flex flex-col items-start justify-between gap-4 sm:flex-row">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {inv.investmentMode === "recurring"
                            ? `${formatCurrency(inv.monthlyAmount)}/mo`
                            : `Date: ${formatDate(inv.date)}`}
                          {inv.status === "closed"
                            ? ` • Closed on ${formatDate(inv.stopDate)}`
                            : ""}
                        </p>
                      </div>
                      <div className="text-left sm:text-right">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {inv.status === "closed" ? "Realized" : "Current"}
                        </p>
                        <p className="font-semibold tabular-nums">
                    ...
                    <div className="flex flex-wrap gap-2 justify-start sm:justify-end">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          inv.investmentMode === "recurring"
                            ? (setPendingDeleteId(inv._id),
                              setFromDate(nowYM()),
                              setFromDateOpen(true))
                            : doDelete(inv._id, null)
                        }
                      >
                        Delete
                      </Button>
          ...
          <form onSubmit={handleEditSubmit} className="space-y-3">
            ...
            {editTarget?.investmentMode === "recurring" ? (
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditData({ ...editData, monthlyAmount: e.target.value })
                  }
                />
              </div>
            ) : (
            ...
            <div>
              ...
              <Input
                ...
                min="0"
                max={MAX_RETURN}
                value={editData.expectedReturnRate || ""}
                onChange={(e) =>
                  setEditData({
                    ...editData,
                    expectedReturnRate: e.target.value,
      ...
      <Dialog open={stopOpen} onOpenChange={setStopOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div>
              <Label>Realized Value (INR)</Label>
              <Input
                type="number"
                min="0"
                value={realizedValue}
            ...
            <Button
              ...
              disabled={submitting}
            >
              {submitting ? "Processing..." : "Confirm Stop/Sell"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
